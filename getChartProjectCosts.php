	...
	mysqli_query($mysqli, "SET NAMES 'utf8'");
        ...
        for ($year = date("Y")-1; $year <= date("Y"); $year++) {            
            for ($month = 1; $month <= 12; $month++) {
                ...
                /*$sql = "SELECT Sum(PP) as sum
                        FROM
                        (
                                SELECT rbchanges3.id, rbchanges3.anzahlNeu, rbchanges3.bestand, rbchanges3.elementID, rbchanges3.variante, variantenCosts2.costs, variantenCosts2.costs*rbchanges3.anzahlNeu AS PP, 
                                                projektgwerke2.gewerk_neu, projektgwerke2.ghg_neu, projektgwerke2.gug_neu
                                FROM 
                                        (
                                                SELECT rbchanges2.id, rbchanges2.tsmax, rbchanges2.aenderung_idmax, tabelle_rb_aenderung.`elementID_neu` AS elementID, tabelle_rb_aenderung.`Anzahl_copy1` AS anzahlNeu, tabelle_rb_aenderung.`Neu/Bestand_copy1` AS bestand, tabelle_rb_aenderung.Standort_copy1 AS standort, tabelle_rb_aenderung.tabelle_Varianten_idtabelle_Varianten_copy1 As variante
                                                FROM tabelle_rb_aenderung
                                                INNER JOIN(
                                                        SELECT rbchanges.id, MAX(rbchanges.ts) AS tsmax, MAX(rbchanges.aenderung_id) AS aenderung_idmax
                                                        FROM
                                                                (SELECT tabelle_rb_aenderung.idtabelle_rb_aenderung as aenderung_id, tabelle_rb_aenderung.id AS id, tabelle_rb_aenderung.Timestamp As ts
                                                                FROM (tabelle_räume INNER JOIN tabelle_räume_has_tabelle_elemente ON tabelle_räume.idTABELLE_Räume = tabelle_räume_has_tabelle_elemente.TABELLE_Räume_idTABELLE_Räume) INNER JOIN tabelle_rb_aenderung ON tabelle_räume_has_tabelle_elemente.id = tabelle_rb_aenderung.id
                                                                WHERE (((tabelle_räume.tabelle_projekte_idTABELLE_Projekte)=".$_SESSION["projectID"].") AND ((tabelle_rb_aenderung.Timestamp) < '".$date."')) ORDER BY tabelle_rb_aenderung.id) 
                                                                AS rbchanges
                                ...
                                ON (rbchanges3.elementID = variantenCosts2.elID AND rbchanges3.variante = variantenCosts2.elVar)
                                LEFT JOIN
                                        (
                                                SELECT projektgewerke.id, projektgewerke.elID, projektgewerke.tsGewerk, 
                                                        `tabelle_projekt_elment_gewerk_aenderung`.`gewerk_neu`,
                                                        `tabelle_projekt_elment_gewerk_aenderung`.`ghg_neu`,
                                                        `tabelle_projekt_elment_gewerk_aenderung`.`gug_neu`
                                                FROM `LIMET_RB`.`tabelle_projekt_elment_gewerk_aenderung`
                                                INNER JOIN
                                                        (
                                                                SELECT MAX(`tabelle_projekt_elment_gewerk_aenderung`.`idtabelle_projekt_elment_gewerk_aenderung`) as id,
                                                                        `tabelle_projekt_elment_gewerk_aenderung`.`element_neu` as elID,
                                                                        MAX(`tabelle_projekt_elment_gewerk_aenderung`.`date`) as tsGewerk
                                                                FROM `LIMET_RB`.`tabelle_projekt_elment_gewerk_aenderung`
                                                                WHERE
                                                                `tabelle_projekt_elment_gewerk_aenderung`.`projekt_neu` = ".$_SESSION["projectID"]." AND
                                                                `tabelle_projekt_elment_gewerk_aenderung`.`date` < '".$date."'
                                                                GROUP BY elID
                                                        ) AS projektgewerke
                                                ON projektgewerke.id = `tabelle_projekt_elment_gewerk_aenderung`.`idtabelle_projekt_elment_gewerk_aenderung`
                                        ) AS projektgwerke2
                                ON rbchanges3.elementID = projektgwerke2.elID
                        )
                        as summe;";
                */

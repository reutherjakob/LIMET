...
$SB = 210 - 2 * $einzugLR;  // A4: seitenbreite minus die lr einzüge = 180
...
$pdf = init_pdf_attributes($pdf, $einzugLR, $marginTop);
...
foreach ($roomIDsArray as $valueOfRoomID) {
    ...
    while ($row = $result_rooms->fetch_assoc()) {
        ...
        raum_header($pdf, $ln_spacer2, $row['Raumbezeichnung'], $row['Raumnr'], $row['Raumbereich Nutzer'], $row['Geschoss'], $row['Bauetappe'], $row['Bauabschnitt']); //utils function 
        ...
        if ($PDF_input_bools[4]) {
            ...
            $SizeElektroSegement = 60 + $block_header_height + 9 * $ln_spacer1;
        ...
        if ($PDF_input_bools[5]) { //HT
            ...
            if (strlen($outstr) > 0 && is_not_no_comment($outstr)) { //Haustechnik anmerkung
                ...
                $pdf->MultiCell($SB - $e_E, $rowHeightComment, $outstr, 0, 'L', 0, 0);
        ...
        if ($PDF_input_bools[8]) { //MT- LISTE
            make_MT_details_table($pdf, $mysqli, $valueOfRoomID, $block_header_height);
        }
    } //sql:fetch-assoc
}// for every room 

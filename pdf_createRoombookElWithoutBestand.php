...
$roomIDs = filter_input(INPUT_GET, 'roomID');
$teile = explode(",", $roomIDs);
$widths = array(15, 15, 20, 10, 90, 20, 10);
...
foreach ($teile as $valueOfRoomID) {
    ...
    $sql2 = "SELECT tabelle_räume.Raumnr, tabelle_räume.Raumbezeichnung,  tabelle_projekte.Projektname, tabelle_planungsphasen.Bezeichnung, tabelle_räume.`Raumbereich Nutzer`, tabelle_räume.Nutzfläche, tabelle_räume.Bauetappe, tabelle_räume.Bauabschnitt, tabelle_räume.Geschoss, `tabelle_räume`.`Anmerkung FunktionBO`
    ...
    while ($row = $result->fetch_assoc()) {
        if ($idRoombookEntry != $row['id']) {
            ...
            if ($row['Neu/Bestand'] == 1) {
                $pdf->MultiCell($widths[6], $rowHeightMainLine, "", 0, 'C', $fill, 0);
            } else {
                $pdf->MultiCell($widths[6], $rowHeightMainLine, "", 0, 'C', $fill, 0);
            }

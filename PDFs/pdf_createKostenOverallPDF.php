...
if ((int)$_SESSION["projectID"] === 80) {
    $pdf->MultiCell(200, '', "", '', 'L', 0, 0);
    $pdf->SetFont('helvetica', 'B', 7);
    $pdf->MultiCell(80, '', "Bereiche", '', 'L', 0, 0);
    $pdf->SetFont('helvetica', 'I', 7);
    $pdf->Ln();
    $pdf->MultiCell(200, '', "", '', 'L', 0, 0);
    $pdf->MultiCell(80, '', "ZAE   = Haus C/D", '', 'L', 0, 0);
    $pdf->Ln();
    $pdf->MultiCell(200, '', "", '', 'L', 0, 0);
    $pdf->MultiCell(80, '', "ZAE*  = U/Bs 1-6 ", '', 'L', 0, 0);
    $pdf->Ln();
    $pdf->MultiCell(200, '', "", '', 'L', 0, 0);
    $pdf->MultiCell(80, '', "ZAE** = Zubau Haus B", '', 'L', 0, 0);
    $pdf->Ln();
...
// Prepare statement with a parameter placeholder '?'
        ...
        RIGHT JOIN (
            ...
            RIGHT JOIN (
                ...
                RIGHT JOIN (
                    ...
                    ON (
                        tabelle_projekt_element_gewerk.tabelle_elemente_idTABELLE_Elemente = tabelle_räume_has_tabelle_elemente.TABELLE_Elemente_idTABELLE_Elemente
                    )
                    AND (
                        tabelle_räume.tabelle_projekte_idTABELLE_Projekte = tabelle_projekt_element_gewerk.tabelle_projekte_idTABELLE_Projekte
                    )
                )
...
$pdf->setXY($xPosition, $yPosition);
$pdf->MultiCell(40, 6, "Bereich", 'B', 'L', 0, 0);
$pdf->MultiCell(30, 6, "Geschoss", 'B', 'C', 0, 0);
...
$sql = "SELECT tabelle_räume.`Raumbereich Nutzer`, tabelle_räume.Geschoss, tabelle_räume.Bauabschnitt
...
foreach ($raumbereicheInProject as $rowData) {
    ...
    if ((int)$_SESSION["projectID"] === 80) {
        $pdf->MultiCell(20, 4, $rowData['Geschoss'] . " (Haus " . $rowData["Bauabschnitt"] . ")", 0, 'L', $fill, 0);
    } else {
        $pdf->MultiCell(20, 4, $rowData['Geschoss'], 0, 'L', $fill, 0);
    }
    ...
    // ------------------------------------Neu --------------------------------------------------
    $pdf->MultiCell(70, 4, 'davon Neu', 0, 'R', $fill, 0);
    ...
    // ------------------------------------Bestand ----------------------------------------------
    $pdf->MultiCell(70, 4, 'davon Bestand', 0, 'R', $fill, 0);
...
$pdf->MultiCell(70, 4, 'davon Neu', 0, 'R', 0, 0);
...
$pdf->Ln();
$pdf->SetFont('helvetica', 'BI', 6);
$pdf->MultiCell(70, 4, 'davon Bestand', 0, 'R', 0, 0);

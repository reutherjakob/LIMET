...
// Safely retrieve and validate integer lotID from GET parameters
$lotID = getPostInt('lotID',0);
if ($lotID === 0) {
    echo json_encode(["error" => "Invalid or missing lotID"]);
    exit;
}

$stmt = $mysqli->prepare(
    "SELECT idtabelle_rechnungen, Nummer, InterneNummer, Ausstellungsdatum, Eingangsdatum, Rechnungssumme, Bearbeiter, Schlussrechnung
     FROM tabelle_rechnungen
     WHERE tabelle_lose_extern_idtabelle_Lose_Extern = ?
     ORDER BY InterneNummer "
);

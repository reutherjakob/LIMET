...
if (!isset($_SESSION["projectID"]) || $elementID <= 0) {
    die("Invalid project or element ID");
}
...
WHERE tabelle_räume.idTABELLE_Räume NOT IN (
    SELECT tr.idTABELLE_Räume
    FROM tabelle_räume tr
    INNER JOIN tabelle_räume_has_tabelle_elemente rhe ON tr.idTABELLE_Räume = rhe.TABELLE_Räume_idTABELLE_Räume
...
$stmt->bind_param("iii", $elementID, $_SESSION["projectID"], $_SESSION["projectID"]);
$stmt->execute();

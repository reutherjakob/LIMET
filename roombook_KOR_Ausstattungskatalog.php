...
<head>
    <title>RB – Elektrische Anschlussübersicht</title>
...
<div class="container-fluid">
    ...
    <div class="mt-2 card">
        <div class="card-header">
            <div class="row">
                <div class="col-6">
                    <b>Elektrische Anschlussübersicht – Elemente im Pro jekt</b>
                </div>
                <div class="col-6 d-flex justify-content-end align-items-center" id="dt-header-controls">
        ...
        <div class="card-body p-1">
            ...
            //   'header'       => <th> label
            //   'source'       => 'display' | 'param' | 'calc_ac' | 'calc_wc' | 'calc_sc'
            //   'key'          => key in the source array
            //   'suppress_unit'=> true: don't append Einheit in display value
            //   'center'       => true: text-center on td
            //   'group'        => logical group label (for comments / future grouping)
            // ---------------------------------------------------------------
            $columnDefs = [
                ...
                ['header' => 'Netzart', 'source' => 'display', 'key' => 'Netzart', 'group' => 'Elektro'],
                ['header' => 'Spannung [V]', 'source' => 'display', 'key' => 'Spannung', 'suppress_unit' => true, 'group' => 'Elektro'],
                ['header' => 'Steckdosen Anz.', 'source' => 'display', 'key' => 'Steckdosen_Anzahl', 'group' => 'Elektro'],
                ['header' => 'Direktanschluss', 'source' => 'display', 'key' => 'Direktanschluss', 'group' => 'Elektro'],
                ...
                ['header' => 'Potentialausgleich', 'source' => 'display', 'key' => 'PA', 'group' => 'Elektro'],
                ['header' => 'IKT Anschluss', 'source' => 'display', 'key' => 'RJ45 Ports', 'group' => 'Elektro'],

                // --- Medizingas ---
                ['header' => 'O2', 'source' => 'display', 'key' => 'O2 Anschluss', 'group' => 'Medizingas'],
                ['header' => 'Med. DL5', 'source' => 'display', 'key' => 'DL-5 Anschluss', 'group' => 'Medizingas'],
                ['header' => 'Med. DL10', 'source' => 'display', 'key' => 'DL-10 Anschluss', 'group' => 'Medizingas'],
                ['header' => 'NGA', 'source' => 'display', 'key' => 'NGA Anschluss', 'group' => 'Medizingas'],
                ['header' => 'VA', 'source' => 'display', 'key' => 'VAC Anschluss', 'group' => 'Medizingas'],
                ['header' => 'CO2', 'source' => 'display', 'key' => 'CO2 Anschluss', 'group' => 'Medizingas'],
                ...
                // --- Kaltwasser ---
                ['header' => 'KW Stadtwasser', 'source' => 'calc_wc', 'key' => 'kw_stadt_flag', 'center' => true, 'group' => 'Kaltwasser'],
                ['header' => 'KW Stadtwasser l/h', 'source' => 'calc_wc', 'key' => 'kw_stadt_strom', 'group' => 'Kaltwasser'],
                ['header' => 'KW weich <4°DH', 'source' => 'calc_wc', 'key' => 'kw_weich_flag', 'center' => true, 'group' => 'Kaltwasser'],
                ['header' => 'KW weich <4°DH l/h', 'source' => 'calc_wc', 'key' => 'kw_weich_strom', 'group' => 'Kaltwasser'],
                ['header' => 'KW weich <4°DH, 80-120 μS/cm', 'source' => 'calc_wc', 'key' => 'kw_weich_leitf_flag', 'center' => true, 'group' => 'Kaltwasser'],
                ['header' => 'KW weich <4°DH, 80-120 μS l/h', 'source' => 'calc_wc', 'key' => 'kw_weich_leitf_strom', 'group' => 'Kaltwasser'],
                ['header' => 'KW VE <0,2 μS + <10 KBE/ml', 'source' => 'calc_wc', 'key' => 'kw_ve02_flag', 'center' => true, 'group' => 'Kaltwasser'],
                ['header' => 'KW VE <0,2 μS l/h', 'source' => 'calc_wc', 'key' => 'kw_ve02_strom', 'group' => 'Kaltwasser'],
                ['header' => 'KW VE <15 μS/cm', 'source' => 'calc_wc', 'key' => 'kw_ve15_flag', 'center' => true, 'group' => 'Kaltwasser'],
                ['header' => 'KW VE <15 μS/cm l/h', 'source' => 'calc_wc', 'key' => 'kw_ve15_strom', 'group' => 'Kaltwasser'],

                // --- Warmwasser ---
                ['header' => 'WW Stadtwasser', 'source' => 'calc_wc', 'key' => 'ww_stadt_flag', 'center' => true, 'group' => 'Warmwasser'],
                ['header' => 'WW Stadtwasser l/h', 'source' => 'calc_wc', 'key' => 'ww_stadt_strom', 'group' => 'Warmwasser'],
                ['header' => 'WW weich <4°DH', 'source' => 'calc_wc', 'key' => 'ww_weich_flag', 'center' => true, 'group' => 'Warmwasser'],
                ['header' => 'WW weich <4°DH l/h', 'source' => 'calc_wc', 'key' => 'ww_weich_strom', 'group' => 'Warmwasser'],
            ...
            // Replace the array_column line AND remove the hardcoded array entirely
            ...
            // Helper: Varianten-ID → Buchstabe (wie in KOR-Elementliste)
            function varianteLetter(int $id): string
            {
                return $id > 0 ? chr(64 + $id) : '';
            }

            function isVersorgungseinheit(string $elementID): bool            // Trifft zu bei: 1.61.xx.xx, 4.35.25.xx, 1.35.13.2, 1.35.13.6
            {
            ...
            function normalize_to_base_unit(string $wert, string $einheit): array
            {
                ...
                $prefixes = [
                    'G' => 1_000_000_000,   // Giga  (GW, GJ, ...)
                    'M' => 1_000_000,       // Mega  (MW, MJ, MVA, ...)
                    'k' => 1_000,           // Kilo  (kW, kV, kVA, ...)
                ];

                // Units that start with a prefix letter but must NOT be converted
                $exceptions = ['kg', 'km', 'kn', 'MHz', 'GHz', 'MB', 'GB'];

                if (is_numeric(str_replace(',', '.', $wert))) {
                    foreach ($prefixes as $prefix => $multiplier) {
                        // Check if unit starts with this prefix (case-sensitive for M/G, lower for k)
                        if (str_starts_with($einheit, $prefix)) {
                            // Skip exceptions
                            if (in_array($einheit, $exceptions, true)) {
                                break;
                            }
                            $baseUnit = substr($einheit, strlen($prefix));
                            $numericVal = (float)str_replace(',', '.', $wert);
                            return [
                                'wert' => (string)($numericVal * $multiplier),
                                'einheit' => $baseUnit
                            ];
                        }
                    }
            ...
            function calc_wasser_cols(array $params, array $display): array
            {
                ...
                $kw_haerte_ok = !$kw_weich; // "nicht ausgewählt / > 4"

                // Restleitfähigkeit: between 80–120
                ...
                $r['anschluss_punkt'] = (strpos($kw_einheit, '"') !== false) ? $kw_anschluss_val : '';
                // --- Rohrtrenner EN1717 ---
            ...
            {
                $r = [];
                // Use $params (clean, no unit) for values where header already states the unit
                $r['raumwaerme_sensibel'] = trim($params['Abwärme'] ?? '');           // [W] in header
                $r['kaelteleistung'] = trim($params['Kühlwasser_Abwärme'] ?? ''); // [W] in header
                $r['kw_vorlauf_temp'] = trim($params['Kühlwasser_Temperatur'] ?? ''); // [°C] in header
                $r['druckverlust'] = trim($params['Kühlwasser_Druckverlust'] ?? ''); // [Pa] in header
                $r['abwasser_temp'] = trim($params['Abflusstemperatur'] ?? '');  // [°C] in header
                $r['abluft_temp'] = trim($params['Ablufttemperatur'] ?? '');   // [°C] in header
                $r['gewicht'] = trim($params['(Eigen-)Gewicht'] ?? '');    // [kg] in header
                $r['laerm'] = trim($params['Lärm'] ?? '');

                $montage_ort = strtolower(trim($params['Montage_Ort'] ?? ''));
                $punktlast = trim($params['Punktlast'] ?? '');                        // [N] in header
                $r['punktlast_decke'] = (stripos($montage_ort, 'decke') !== false) ? $punktlast : '';
                $r['punktlast_boden'] = (stripos($montage_ort, 'boden') !== false) ? $punktlast : '';

                // Everything else still uses $display (unit is useful there)
                ...
                $r['abwasser_strom'] = trim($display['Abflussstrom'] ?? '');
                $r['kw_vorlauf_anschluss'] = trim($display['Kühlwasser_Vorlauf_Anschluss'] ?? '');
                $r['kw_ruecklauf_anschluss'] = trim($display['Kühlwasser_Rücklauf_Anschluss'] ?? '');
                $r['abluft_menge'] = trim($display['Abluftstrom'] ?? '');
                $r['abluft_dimension'] = trim($display['Abluftdurchmesser'] ?? '');
                $r['abwasser_dimension'] = trim($display['Abflussdurchmesser'] ?? '');
            ...
            // 1) Alle Elemente des Projekts (mind. 1x in einem Raum)
            $sqlElements = "
                    SELECT 
            ...
            // ---------------------------------------------------------------
            // 2) Alle Parameter dieser Elemente auf einmal laden
            // ---------------------------------------------------------------
            $elementIDs = array_column($elements, 'idTABELLE_Elemente');
            $paramMap = [];   // [elemID][paramBezeichnung] = Wert
            if (!empty($elementIDs)) {
                ...
                $types = str_repeat('i', count($params));  // if all are ints
                $stmtP = $mysqli->prepare($sqlParams);
                $stmtP->bind_param($types, ...$params);
                $stmtP->execute();
                $resParams = $stmtP->get_result();
                $paramMap = [];   // clean Wert for logic
                $paramDisplay = [];  // Wert + Einheit for display
                ...
                while ($prow = $resParams->fetch_assoc()) {
                    ...
                    // Suppress unit in display if column header already states it
            ...
            <div style="overflow-x: auto;">
                <table class="table table-striped table-hover table-bordered table-sm compact"
                    ...
                    foreach ($columnDefs as $col) {
                        ...
                        echo "<th{$hidden}>" . htmlspecialchars($col['header'], ENT_QUOTES, 'UTF-8') . "</th>";
                    }
                    echo "</tr></thead>";
                    ...
                    foreach ($elements as $elem) {
                        ...
                        $out_ortsv = ($elem['GewerkBezeichnung'] === 'ORT') ? 'Ja' : 'Nein';
                        $ve = isVersorgungseinheit($elem['ElementID']) ? '1' : '';
    ...
    <?php
    include "modal_Kor_ausstattungkatalog_info.php"
    ?>

...
// A: Seite Halbieren; B: Seite dritteln; C:Seite/4; E=1/5; F = 1/6    -> weil auf A3, jeweils halbierzt. 
//$e_A = $SB / 4;
//$e_A_3rd = $e_A / 3;
...
foreach ($roomIDsArray as $valueOfRoomID) {
    ...
    while ($row = $result_rooms->fetch_assoc()) {
        ...
        if (strlen($row['Anmerkung FunktionBO']) > 0) {
            $einzugPlus = 15;// um den text auf die Höhe der Anderen Angaben zu shiften
            ...
            $rowHeightComment = $pdf->getStringHeight($SB-$einzugPlus0, $outstr, false, true, '', 1);
        ...
        //   ---------- ALLGEMEIN   ----------
        //
        block_label_queer($block_header_w, $pdf, "Allgemein", $block_header_height, $SB);
        $pdf->MultiCell($e_B_2_3rd, 6, "Hygieneklasse: ", 0, 'R', 0, 0);
        ...
        $pdf->MultiCell($e_B_2_3rd, 6, "OENORM B5220: ", 0, 'R', 0, 0);
        ...
        if (($pdf->getStringHeight($e_B_3rd, $row['Fussboden OENORM B5220'])) > 6 || ($pdf->getStringHeight($e_B_3rd, $row['Strahlenanwendung'])) > 6) {
            $pdf->Ln(6);
        }
        $pdf->Ln($horizontalSpacerLN);
        ...
        multicell_with_nr($pdf, $row['Raumhoehe'], "m", 10, $e_B_3rd,);
        ...
        multicell_with_str($pdf, $row['Umfang'], $e_B_3rd, "m");
        ...
        $pdf->MultiCell($e_B_2_3rd, 6, "Laseranw.: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['Laseranwendung'], "JA");
        $pdf->MultiCell($e_B_2_3rd, 6, "Abdunkelbarkeit: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['Abdunkelbarkeit'], "JA");
        ...
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['Aufenthaltsraum'], "JA");
        
        //
        //    ---------- ELEKTRO -----------
        //
        $pdf->Ln($horizontalSpacerLN3);
        block_label_queer($block_header_w, $pdf, "Elektro", $block_header_height, $SB);

        $pdf->MultiCell($e_B_2_3rd, 6, "ÖVE E8101:", 0, 'R', 0, 0);
        multicell_with_str($pdf, $row['Anwendungsgruppe'], $e_B_3rd, "");
        $pdf->MultiCell($e_B_2_3rd, 6, "AV: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['AV'], "JA");
        $pdf->MultiCell($e_B_2_3rd, 6, "SV: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['SV'], "JA");
        $pdf->MultiCell($e_B_2_3rd, 6, "ZSV: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['ZSV'], "JA");
        $pdf->MultiCell($e_B_2_3rd, 6, "USV: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['USV'], "JA");

        $pdf->Ln($horizontalSpacerLN);
        $pdf->MultiCell($block_header_w, 10, "", 0, 'L', 0, 0);

        $pdf->MultiCell($e_B_2_3rd, 6, "IT: ", 0, 'R', 0, 0);
        hackerl($pdf, $hackerl_schriftgröße, $e_B_3rd, $row['IT Anbindung'], "JA");

        $pdf->MultiCell($e_B_2_3rd, 6, "Anschlussleistung: ", 0, 'R', 0, 0);
        multicell_with_str($pdf, " TODO ", $e_B_3rd, "W");
        $pdf->MultiCell($e_B_2_3rd, 6, "Anschlussleistung: ", 0, 'R', 0, 0);
        multicell_with_str($pdf, " TODO ", $e_B_3rd, "W");
        $pdf->MultiCell($e_B_2_3rd, 6, "Anschlussleistung: ", 0, 'R', 0, 0);
        multicell_with_str($pdf, " TODO ", $e_B_3rd, "W");
        ...
        //
        $pdf->Ln($horizontalSpacerLN3);
        block_label_queer($block_header_w, $pdf, "Haustechnik", $block_header_height, $SB);

        $pdf->MultiCell($e_B_2_3rd, 6, "H6020: ", 0, 'R', 0, 0);
        multicell_with_str($pdf, $row['H6020'], $e_B_3rd, "");
        $pdf->MultiCell($e_B_2_3rd, 6, "Abwärme: ", 0, 'R', 0, 0);
        $abwärem_out= $row['HT_Waermeabgabe_W'];
        if($abwärem_out == "0"){$abwärem_out= "keine Angabe";}else{$abwärem_out = $abwärem_out +" W";}
        multicell_with_str($pdf, $abwärem_out , 4*$e_B_3rd, "");
        ...
        if (strlen($outstr) > 0 && is_not_no_comment($outstr)) { //Haustechnik anmerkung 
            $pdf->Ln($horizontalSpacerLN);
            $rowHeightComment = $pdf->getStringHeight($SB - $e_B_2_3rd - $block_header_w, $outstr, false, true, '', 1);
            $pdf->MultiCell($e_B_2_3rd + $block_header_w, $rowHeightComment, "Anmerkung:", 0, 'R', 0, 0);
            $pdf->MultiCell($SB - $e_B_2_3rd - $block_header_w, $rowHeightComment, $outstr, 0, 'L', 0, 0);
            if ($rowHeightComment > 6) {
                $pdf->Ln($horizontalSpacerLN);
            }
        }
        ...
        //
        $pdf->Ln($horizontalSpacerLN3);
        block_label_queer($block_header_w, $pdf, "Med.-Gas", $block_header_height, $SB);

        $pdf->MultiCell($e_B_2_3rd, 6, "1 Kreis O2: ", 0, 'R', 0, 0);
        ...
        $pdf->MultiCell($e_B_2_3rd, 6, "1 Kreis DL5: ", 0, 'R', 0, 0);
        ...
        if (strlen($row['AR_Ausstattung']) > 0) {
            $pdf->Ln($horizontalSpacerLN3);
            block_label_queer($block_header_w, $pdf, "Architektur-Einrichtung", $block_header_height, $SB);
            $rowHeightComment = $pdf->getStringHeight($SB - $block_header_w, br2nl($row['AR_Ausstattung']), false, true, '', 1);
            check_4_new_page($pdf, $rowHeightComment);
            $pdf->MultiCell($SB - $block_header_w, $rowHeightComment, br2nl($row['AR_Ausstattung']), 0, 'L', 0, 1);
        }
        ...
        make_MT_details_table($pdf, $mysqli, $valueOfRoomID, $block_header_height, $SB, $SH);
    } //sql:fetch-assoc
}// for every room 
